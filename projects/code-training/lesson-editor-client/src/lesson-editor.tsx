...
export const LessonEditor = (props: {
  value: LessonData;
  onChange: (value: LessonData) => void;
  ...
  const projectEditorMode: LessonProjectEditorMode = editorMode === `edit` ? `edit` : `display`;
  const fileEditorMode_focus: LessonFileEditorMode =
    editorMode === `edit` ? `edit` : editorMode === `construct-code` ? `construct-code` : `display`;
  const fileEditorMode_noFocus: LessonFileEditorMode =
    editorMode === `edit` ? `edit` : editorMode === `construct-code` ? `display` : `display`;
  ...
  const { projectState, focus } = data.lesson;
          ...
          {editorModes.map((x) => (
            <TouchableOpacity key={x.value + editorMode} onPress={() => setEditorMode(x.value)}>
              <View style={x.value === editorMode ? styles.editorModeTabView_selected : styles.editorModeTabView}>
                <Text style={x.value === editorMode ? styles.editorModeTabText_selected : styles.editorModeTabText}>
                  {x.label}
                </Text>
              </View>
            </TouchableOpacity>
        ...
        <View style={styles.containerPanel}>
          ...
          {editorMode === `edit` && (
            <>
              ...
              <View style={styles.infoView}>
                ...
                <LessonField
                  label='Objective'
                  value={data.lesson.objective}
                  onChange={(x) => onLessonChange({ objective: x })}
                />
                <LessonField
                  label='Explanation'
                  value={data.lesson.explanation}
                  onChange={(x) => onLessonChange({ explanation: x })}
                />
                <LessonField label='Task' value={data.lesson.task} onChange={(x) => onLessonChange({ task: x })} />
                <LessonField_Descriptions
                  label='Descriptions'
                  value={data.lesson.descriptions}
                  onChange={(x) => onLessonChange({ descriptions: x })}
                />
                <LessonField_Experiments
                  label='Experiments'
                  value={data.lesson.experiments}
                  onChange={(x) => onLessonChange({ experiments: x })}
                  lessonData={data.lesson}
                />
              </View>
            </>
          )}
          {editorMode === `construct-code` && <LessonView_ConstructCode data={data.lesson} />}
          {editorMode === `understand-code` && <LessonView_UnderstandCode data={data.lesson} />}
          ...
          {editorMode === `preview` && (
            <LessonProjectStatePreview
              projectState={data.lesson.projectState}
...
const LessonField_Descriptions = ({
  label,
  value,
  onChange,
}: {
...
const LessonField_Experiments = ({
  ...
  return (
    <View style={{}}>
      ...
      <TouchableOpacity
        onPress={() => {
          value.push({ replacements: [], comment: ``, filesHashCode: calculateFilesHashCode([]) });
          onChange(value);
        }}
      >
...
const LessonField_Experiment = ({
  ...
  const [lastFocus, setLastFocus] = useState({
    filePath: modifiedProjectState.files[0].path,
    index: 0,
    length: modifiedProjectState.files[0].content.length,
  });
  const [commentText, setCommentText] = useState(value.comment ?? ``);

  useEffect(() => {
    setModifiedProjectState((s) =>
      lessonExperiments_createReplacementProjectState(lessonData.projectState, value.replacements),
    );
  ...
  const changeProjectData = (data: { projectState?: LessonProjectState; focus?: LessonProjectFileSelection }) => {
    ...
    onChange({
      ...lessonExperiments_calculateProjectStateReplacements(lessonData.projectState, data.projectState),
      comment: commentText,
      filesHashCode: calculateFilesHashCode(lessonData.projectState.files),
    });
  };
      ...
      {value.replacements.map((x) => (
        <View key={x.selection.filePath + x.selection.index}>
          ...
          <View style={{ flexDirection: `row` }}>
            ...
            <Text
              style={{ marginLeft: 4, background: `#111111`, color: `#FF8888`, textDecoration: `line-through` }}
            >{`${lessonData.projectState.files
              .find((f) => f.path === x.selection.filePath)
              ?.content.substr(x.selection.index, x.selection.length)}`}</Text>
          </View>
          <View style={{ flexDirection: `row` }}>
