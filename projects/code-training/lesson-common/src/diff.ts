...
const getSpanRange = (v: {
  span: StringSpan;
  lineSpans: StringSpan[];
  startLine: number;
  endLine: number;
  startColumn?: number;
  endColumn?: number;
  otherAtStart: boolean;
  otherAtEnd: boolean;
}) => {
...
export const diff = (a: string, b: string) => {
  ...
  const diffComputer = new DiffComputer(
    aLineSpans.map((x) => x.toText()),
    bLineSpans.map((x) => x.toText()),
    options,
  );
  const { changes: lineChanges } = diffComputer.computeDiff();
  ...
  const charChanges = lineChanges.flatMap(
    (x) =>
      (x.charChanges?.map((c) => ({
        line: x,
        ...c,
  ...
  const changes = charChanges.map((x) => {
    if (
      (x.originalStartColumn === 0 && x.originalEndColumn === 0) ||
  ...
  return {
    changes: distinct_key(
      changes,
      (x) => `${x.a.change.start}:${x.a.change.length}=>${x.b.change.start}:${x.b.change.length}`,
    ),
  };
};
