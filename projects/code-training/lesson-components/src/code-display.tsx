...
export const CodeDisplay = ({
  ...
  const Cursor = useMemo(
    () => <CursorComponent isActive={inputOptions?.isActive ?? false} />,
    ...
    <View>
      ...
      <pre style={{ margin: 0, paddingBottom: inputOptions ? 100 : 0 }} className={`language-${language}`}>
        <code className={`language-${language}`}>
          <CodeSpan
            key='-1'
            code={{ code: `\n`, index: -1, indexAfterEnd: 0, length: 1, classes: [], isInSelection: false }}
            Cursor={Cursor}
            inputOptions={inputOptions ?? {}}
          />
          {codeParts.map((x) => (
            <CodeSpan
              key={`${x.code}:${x.index}:${x.code.length}`}
              code={x}
              Cursor={Cursor}
              inputOptions={inputOptions ?? {}}
            />
...
const CodeSpan = ({
  ...
  code: CodeDisplayPart;
  Cursor: JSX.Element;
  inputOptions: CodeDisplayInputOptions;
}) => {
  const { cursorIndex, activeIndex = inputOptions.cursorIndex, promptIndex } = inputOptions;

  const hasCursor = !!(cursorIndex != null && cursorIndex >= code.index && cursorIndex < code.index + code.code.length);
  const hasActive = !!(activeIndex != null && activeIndex >= code.index && activeIndex < code.index + code.code.length);
  const hasPrompt = !!(promptIndex != null && promptIndex >= code.index && promptIndex < code.index + code.code.length);
  return (
    <>
      {hasCursor && Cursor}
      {hasActive && (
      ...
      {!code.onPress && (
        <span
          className={code.classes.join(` `)}
          style={!code.isInSelection ? { opacity: 0.5 } : {}}
        >{`${code.code}`}</span>
      )}
      {code.onPress && (
        ...
        <span style={s.inner}>
          {autoComplete.map((x) => (
            <TouchableOpacity
              key={x.text}
              onPress={() => {
                inputOptions.onAutocomplete?.(x.text);
              }}
            >
              <span style={x.isSelected ? s.item_selected : s.item}>
                <span style={s.textCompleted}>
                  {x.isWrong ? `❌ ` : ``}
                  {x.textCompleted}
                </span>
