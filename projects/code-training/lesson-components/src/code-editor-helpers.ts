...
const parseHighlightedSpans = (html: string): CodeWithClasses[] => {
  ...
  const tagsWithClasses = tagsWithContext.map((t) => {
    const classInfos = t.context.map((c) => {
      ...
      if (!classContent) {
        return { raw: c, cParts, classContentParts };
      }
  ...
  const codeWithClasses = tagsWithClasses
    .map((x) => ({
      code: x.code.toText().replace(/&lt;/g, `<`).replace(/&gt;/g, `>`).replace(/&amp;/g, `&`),
      classes: x.classes,
    }))
    .filter((x) => x.code);

  console.log(`parseHighlightedSpans`, {
    codeWithClasses,
    tagsWithClasses,
    summary: codeWithClasses.map((x) => `<span class='${x.classes.join(` `)}'>${x.code}</span>`).join(``),
  });
...
export const getCodeParts = (code: string, language: 'tsx', selection?: LessonProjectFileSelection): CodePartsData => {
  ...
  if (codeParts.some((x) => x.code !== x._rawCode)) {
    console.error(`getCodeParts FAILED`, {
      codeParts: codeWithClasses,
      codePartsFailed: codeParts.filter((x) => x.code !== x._rawCode),
    });
  }
  ...
  const codePartsAll = modifyCodeParts(
    codeParts.map((x) => ({ ...x, isInSelection: true, indexAfterEnd: x.index + x.length })),
    s,
    (x, inRange) => ({ ...x, isInSelection: inRange }),
  );
...
const modifyCodeParts = <T extends CodePart>(
  codeParts: CodePart[],
  range: { index: number; length: number },
  getCodePart: (codePart: CodePart, inRange: boolean) => T,
): T[] => {
  const s = {
    ...range,
...
export const getCodePartsCompleted = (
  codeParts: CodePart[],
  incomplete: { index: number; length: number },
  options?: { showBlank?: boolean },
): CodePart[] => {
...
export const getAutoComplete = (
  { codeParts, selection }: { codeParts: CodePart[]; selection?: { index: number; length: number } },
  codeFocusCompleted?: string,
) => {
  if (!selection || codeFocusCompleted == null || selection.length === codeFocusCompleted.length) {
    return null;
  }
