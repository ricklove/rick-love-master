...
export const LessonFileContentEditor_ConstructCode = ({
  code,
  language,
  selection,
  onDone,
  ...
  const [autoComplete, setAutoComplete] = useState(
    null as null | {
      choices: { textCompleted: string; text: string; isSelected: boolean; isWrong: boolean }[];
      activeIndex: number;
    },
  );
  ...
  const changeInputText = (valueRaw: string) => {
    ...
    const value2 = wasCorrectRaw
      ? valueRaw
      : wasAutoComplete
      ? inputText + (activeAutoComplete?.text ?? ``)
      : (wasReturn || wasTab || wasSpace) && isWhitespace
      ? codeFocus.substr(0, codeFocus.length - remaining.trimStart().length)
      : valueRaw;
    const wasCorrect_ignoreWhitespace = remaining.trimStart().startsWith(newInput);
    const value = wasCorrect_ignoreWhitespace
      ? codeFocus.substr(0, codeFocus.length - remaining.trimStart().length + newInput.length)
      : value2;
    ...
    if (wasBackspace) {
      setFeedback({
        ...
        message: `❌ You're right so far, no need to backspace.`,
        timestamp: Date.now(),
      });
      return;
    }
  ...
  const onAutocomplete = (text: string) => {
    ...
    autoComplete.choices.forEach((x) => {
      x.isSelected = false;
    });
  ...
  const activeCodeParts = getCodePartsCompleted(
    codeParts.codeParts,
    { index: cursorIndex, length: codeParts.codeFocus.length - inputText.length },
    { showBlank: true },
  );
  ...
  console.log(`CodeEditor_TypeSelection`, {
    nextChar,
    inputText,
    codeParts,
    activeCodeParts,
