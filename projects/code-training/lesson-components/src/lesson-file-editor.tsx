...
import { calculateFilesHashCode } from '@ricklove/code-training-lesson-common';
import { TextInput, View } from '@ricklove/react-native-lite';
import { CodeDisplay } from './code-display';
import { CodePartsData, getCodeParts } from './code-editor-helpers';
...
export const LessonProjectFilesEditor = ({
  ...
  onProjectDataChange?: (value: { projectState?: LessonProjectState; focus?: LessonProjectFileSelection }) => void;
  onTaskDone?: () => void;
  lessonData: LessonData;
}) => {
  const { projectState, focus } = projectData;
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!focusFile || focus.length <= 0 || focus.index >= (focusFile?.content.length ?? 0)) &&
    ...
    <>
      <TabsListEditorComponent
        ...
        onChange={
          projectEditorMode !== `edit`
            ? undefined
            : (x) => onProjectDataChange({ projectState: { files: x, filesHashCode: calculateFilesHashCode(x) } })
        }
        getKey={(x) => x.path}
      ...
      {activeFile && (
        <LessonFileEditor
          ...
          file={activeFile}
          selection={focus.filePath === activeFile.path ? focus : undefined}
  ...
  return (
    <View style={{}}>
      ...
      <View style={{ padding: 0 }}>
        ...
        {mode === `construct-code` && (
          <LessonFileContentEditor_ConstructCode
            code={file.content}
            language={file.language}
            selection={selection}
            onDone={onTaskDone}
        ...
        {mode === `understand-code` && (
          <LessonFileContentEditor_UnderstandCode
            code={file.content}
            language={file.language}
            selection={selection}
            onDone={onTaskDone}
...
const LessonFileContentEditor_Display = ({
  code,
  language,
  selection,
}: {
  ...
  return <CodeDisplay codeParts={codeParts.codeParts} language={language} />;
};

const LessonFileContentEditor_Edit = ({
  code,
  language,
  selection,
  onCodeChange,
  ...
  return (
    <View>
      ...
      <View>{codeParts && <CodeDisplay codeParts={codeParts.codeParts} language={language} />}</View>
    </View>
  );
};
