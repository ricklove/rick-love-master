...
import { OnchainSvgNftContract } from '../typechain/OnchainSvgNftContract';

describe(`OnchainSvgNftContract`, async () => {
  let contract = null as unknown as OnchainSvgNftContract;
...
'/></svg>"}
  */
  ...
  before(async () => {
    const NftProjects = await ethers.getContractFactory(`OnchainSvgNftContract`);
    contract = (await NftProjects.deploy()) as OnchainSvgNftContract;
