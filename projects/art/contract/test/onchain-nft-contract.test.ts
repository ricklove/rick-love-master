...
describe(`OnchainNftContract`, async () => {
  ...
  const tokenJsonParts = tokenJson.split('{SPLIT}');
  const tokenJson_beforeName = tokenJsonParts[0];
  const tokenJson_afterNameBeforeImage = tokenJsonParts[1];
  const tokenJson_afterImage = tokenJsonParts[2];
