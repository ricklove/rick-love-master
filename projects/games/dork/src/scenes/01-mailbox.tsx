...
import { GameAction, GameInput, GameItem, GameScene } from '../types';
...
export const createScene_01mailbox = (gameState: GameState) => {
  ...
  const mailObjects = {
    litterBox: createGameObject(
      `Self Cleaning Litter Box`,
      `Cause 18 years is great even if you have to deal with a little crap sometimes. Keep it clean and happy!`,
      {},
    ),
    ...
    squirrel: createGameObject(
      `Squirrel Stuffed Animal with Nuts`,
      `It looks like you should be careful not to touch it's nuts!`,
      {
        execute: async ({ command, target, onMessage }) => {
          if (command === `touch` && target.includes(`nuts`)) {
            ...
            return await triggerTimedMessage(
              onMessage,
              {
                output: randomItem([
              ...
              5,
              `danger`,
              async () => await triggerGameOver(onMessage, `Don't be touchin my nutz!`),
            );
          }
          return null;
        },
    ...
    tickingPackage: createGameObject(`Ticking Package`, `Ummmm... it's ticking`, {
      execute: async ({ command, onMessage }) => {
        if (command === `open`) {
          ...
          return await triggerGameOver(
            onMessage,
            randomItem([
              `You have Exploded!`,
              `You're head acksplod!`,
              `You no longer hear ticking... probably because your head is gone.`,
            ]),
          );
        }
        return null;
  ...
  const pickupTruck = {
    ...
    description: () => `The crashed pickup truck is smoking. 
        ...
        In the bed of the truck are ${[
          ...
          ...pickupTruck.contents.map((x) => x.titleWithA),
        ].join(`, `)}, and... ${randomItem([`a bunch of beer cans`, `some empty cigarette packages`, `an old tire`])}.
        `,
    hasCrashed: false,
      ...
      return await triggerGameOver(
        ...
        `
                You see ${randomItem([
                  ...
                  `an ambulance`,
                  `a cop car`,
                  `the van from down by the river`,
                  `the ice cream truck`,
                ])} drive up.
      ...
      return {
        output: `
                You see ${randomItem([
                  ...
                  `an Amazon truck`,
                  `the ice cream truck`,
                ])} drive up and a driver wearing a trench coat get out.
                
                He carries a package to the mail box, but when he opens it, he looks suprised.

                He looks around suspiciously... and then puts the ${
                  item.title
                } under his trenchcoat as he slips the new package into the mailbox.
    ...
    return {
      output: `
            You see ${randomItem([
              ...
              `the van from down by the river`,
              `the ice cream truck`,
            ])} drive up, and the driver puts a package in the mailbox.
            He is wearing a ${randomItem([
              `football helmet`,
              `Santa hat`,
              `bow tie`,
              `hazmat suit`,
            ...
            As he drives away, he shouts, "${randomItem([
  ...
  const closeMailbox = async (input: GameInput): Promise<GameAction> => {
    ...
    return triggerTimedMessage(input.onMessage, { output: `You close the mailbox.` }, 10, `normal`, () =>
      deliverMail(input),
    );
  };
  ...
  const execute = async (input: GameInput): Promise<GameAction> => {
    ...
    if (command === `open` && target === `mailbox`) {
      ...
      return {
        output: mailbox.package
          ? `You see a ${mailbox.package.title} in the mailbox.`
    ...
    if (command === `take` && mailbox.package && isMatch(mailbox.package, target)) {
      ...
      if (p === tickingPackage) {
        ...
        return {
          Component: () =>
            (inventory.includes(tickingPackage) && (
              <CountDownTimer
                time={180}
                color='#FF0000'
                onTimeElapsed={async () => {
                  if (!inventory.includes(tickingPackage)) {
                    return;
                  }
                  ...
                  onMessage(
                    await triggerGameOver(
                      onMessage,
                      `You obviously need to watch more TV. A ticking package is generally bad news.`,
                    ),
                  );
                }}
              />
            )) || <span />,
          output: `${!wasOpen ? `You open the mailbox and` : `You`} take the ${
            p.title
          }. As you place it carefully in your backpack, you notice the ticking is getting louder.`,
        };
      }
    ...
    if (
      command === `throw` &&
      inventory.includes(snake) &&
      yard.contents.includes(dog) &&
      isMatch(dog, target) &&
      isMatch(snake, target)
    ) {
      const p = dog;
      moveItem(p, yard.contents, inventory);
      ...
      if (target.includes(`mailbox`)) {
        ...
        if (f) {
          ...
          return await triggerTimedMessage(
            input.onMessage,
            {
              output: `${!wasOpen ? `You open the mailbox and` : `You`} put the ${f.title} in the mailbox ${
                !wasOpen ? `and close it again.` : `and close it.`
              }`,
            },
            10,
            `normal`,
  ...
  const gameScene: GameScene = {
    introduction: `# The Mailbox
        ...
        `,
    execute,
    getLookItems: () => [mailbox, yard, pickupTruck.hasCrashed ? pickupTruck : null],
  };
