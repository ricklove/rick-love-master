...
export const createDorkGame = (onMessageInit: (message: GameAction) => void): Game => {
  ...
  const { isGameOver, inventory, isMatch } = gameState;
  ...
  const scenes = [createScene_01mailbox(gameState)];
  ...
  const execute: GameExecute = async (inputRaw: GameInput): Promise<GameAction> => {
    ...
    command =
      command === `look` || command === `see` || command === `view` || command === `observer` ? `look` : command;
  ...
  const game: Game = {
    ...
    achievements: {
      setValue: (achievements: string[]) => {
        gameState.achievements.loadAchievements(achievements);
      },
      getValue: () => {
        return gameState.achievements.getAchievements();
      },
    },
  };
