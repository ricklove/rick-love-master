...
import { CountDownTimer, triggerTimedMessage } from './components/count-down-timer';
import { GameAction, GameItem, GameItemTitle } from './types';
import { delay, getValuesAsItems, moveItem, randomIndex, randomItem } from './utils';

export const createGameState = (onMessageInit: (message: GameAction) => void) => {
  const _achievements = [] as string[];
  const loadAchievements = (value: string[]) => {
    _achievements.splice(0, _achievements.length, ...value);
  };
  ...
  const createGameObject = (title: string, description: string, options: Partial<GameItem>): GameItem => {
    const g = {
      title,
      titleWithA: getTitleWithA(title),
      description,
      matches: title.toLowerCase().split(` `),
