...
import { defaultDoodleDrawing, DoodleData, DoodleDrawing } from '@ricklove/doodle-common';
...
export const DoodleGameView_DrawWord = (props: {
  prompt: string;
  hint?: string;
  onDone: (drawing: DoodleDrawing) => void;
  onSkip?: () => void;
}) => {
...
export const DoodleGameView_ChooseBest = (props: {
  prompt: string;
  drawings: DoodleData[];
  onChooseBest: (drawing: DoodleData) => void;
}) => {
  return (
    <View style={styles.container}>
      <View style={styles.titleView}>
...
export const DoodleGameView_TypeExpected = (props: {
  prompt: string;
  drawings: DoodleData[];
  onDone: () => void;
  sayAgain: () => void;
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardSimplified
        expectedCharacter={status.remaining[0] ?? ` `}
        showHints
        onExpectedKeyPress={onExpectedKeyPress}
      />
    </View>
  );
};
