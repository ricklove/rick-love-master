...
import { createWebMeshClient, WebMeshClientWebSocketHistory } from '@ricklove/web-mesh';
...
const createPlayerAssignment = (meshState: MeshState, playerClientKey: string, previousAssigments: Assignment[]) => {
  ...
  const playerPrompts = new Set(
    meshState.history.rounds
  ...
  const iRemaining = previousAssigments.findIndex(
    (x) =>
...
const sendNewAssignmentsIfReady = (
  ...
  if (meshState.players.filter((x) => x.isActive && x.isReady).length <= 0) {
    return;
  }

  // Assign Next Item in Chain
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const chains = toKeyValueArray(
    groupItems(
      meshState.history.rounds
        .flatMap((x) => x.completed)
        .map((x) => x.assignment)
  ...
  if (
    meshState.players.some(
      (x) => x.isActive && x.isReady && x.assignment && (!x.assignment.doodle || !x.assignment.prompt),
    )
  ) {
    // Add new player assignments
    const missingAssignments = meshState.players.filter((x) => !x.assignment);
    if (missingAssignments.length > 0) {
      missingAssignments.forEach((x) => {
        x.assignment = createPlayerAssignment(meshState, x.clientKey, previousAssigments);
      });
    ...
    ...meshState.players
      .filter(
        (x) =>
          ...
          x.assignment.doodle &&
          x.assignment.prompt &&
          decodeDoodleDrawing(x.assignment.doodle).segments.length > 0,
      )
  ...
  for (let i = 0; i < meshState.players.length; i++) {
    meshState.players[i].assignment = createPlayerAssignment(
      meshState,
      meshState.players[i].clientKey,
      previousAssigments,
    );
  }
...
const reduceClientsState = (
  previousState: MeshState,
  ...
  previousState.clients = distinct([...previousState.clients, ...clients].map((x) => x.key)).map((x) => ({
    key: x,
    isActive: !!clients.find((c) => c.key === x),
  }));
...
export const useDoodlePartyController = (config: DoodleApiConfig) => {
  ...
  useEffect(() => {
    ...
    const hostIntervalId = setInterval(() => {
      ...
      if (mState.hostClientKey !== webMeshClient.clientKey) {
        return;
      }

      // Act as host
      sendNewAssignmentsIfReady(config, mState, webMeshClient.sendMessage);
  ...
  const sendClientPlayer = () => {
    ...
    if (clientState.client.role !== `player`) {
      return;
    }
  ...
  return {
    ...
    setClientPlayer,
    sendAssignment,
    _messages: (webSocketHistory.current?.history.messages ?? []) as WebMeshClientWebSocketHistory['messages'],
    _events: (webSocketHistory.current?.history.events ?? []) as WebsocketConnectionEvent[],
  };
};
