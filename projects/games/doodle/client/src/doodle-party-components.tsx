      ...
      <View key={clientPlayer.clientKey} style={{ padding: 4, flexDirection: `row`, alignItems: `center` }}>
        ...
        <View>
          <Text style={{ fontSize: 16 }}>
            {meshState?.hostClientKey === clientState.client.clientPlayer.clientKey ? `🟢` : ``}
          </Text>
        </View>
      </View>
    </>
...
export const PartyViewer = (props: { controller: DoodlePartyController }) => {
  const allItems =
    props.controller.meshState?.history.rounds.flatMap((x, iRound) =>
      x.completed.map((y) => ({ iRound, item: y, chainKey: y.assignment?.chainKey })),
    ) ?? [];
  const chains = toKeyValueArray(groupItems(allItems, (x) => x.chainKey ?? ``)).map((x) => ({
    chain: x.key,
    items: x.value.sort((a, b) => a.iRound - b.iRound),
  }));
...
const AssignmentView = (props: { player: PlayerState }) => {
  ...
  return (
    <View style={{ flexDirection: `column`, alignItems: `center`, width: 104 }}>
      <Text>{p.name}</Text>
      <Text>{p.emoji}</Text>
      <Text style={{ color: `#FFFF00`, whiteSpace: `pre-wrap` }}>
        {assignment?.kind === `doodle` ? assignment?.prompt ?? `` : ``}
      </Text>
      {!!assignment?.doodle && (
        <DoodleDisplayView
          style={{ width: 104, height: 104, color: `#FFFFFF`, backgroundColor: `#000000` }}
          drawing={decodeDoodleDrawing(assignment.doodle)}
          shouldAnimate
          enableRedraw
        />
      )}
      <Text style={{ color: `#FFFF00`, whiteSpace: `pre-wrap` }}>
        {assignment?.kind === `describe` ? assignment?.prompt ?? `` : ``}
      </Text>
    </View>
  );
};
...
export const DoodlePartyPlayView = (props: { controller: DoodlePartyController }) => {
    ...
    return (
      <>
        <View style={{ flexDirection: `column`, alignItems: `center` }}>
          ...
          <DoodleDisplayView
            style={{ width: 312, height: 312, color: `#FFFFFF`, backgroundColor: `#000000` }}
            drawing={decodeDoodleDrawing(assigment.doodle)}
            shouldAnimate
            enableRedraw
          />
          {!assigment.prompt && (
  ...
  return (
    <>
      <Text style={{ fontSize: 20, margin: 8 }}>Draw</Text>
      <DoodleGameView_DrawWord
        prompt={assigment.prompt ?? ``}
        onDone={(x) => {
