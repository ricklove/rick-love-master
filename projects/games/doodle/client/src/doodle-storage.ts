...
export const createDoodleDrawingStorageService = async (config: DoodleConfig) => {
  ...
  // Load from server
  memory.doodles =
    (await remoteDoodle.load())?.doodles.map((x) => ({
      key: x.k,
      prompt: x.p,
      timestamp: x.t,
  ...
  summaryData.doodles.forEach((x) => {
    voteTotals[x.key] = (voteTotals[x.key] ?? 0) + x.score;
  });
  ...
  const service: DoodleDrawingStorageService = {
    saveDrawing: async (prompt, drawing) => {
      const doodle = {
        key: `${prompt.substr(0, 8)}:${Date.now()}:${Math.floor(Math.random() * 999999)}`,
        drawing,
        prompt,
        timestamp: Date.now(),
      };
    ...
    getDrawings: async (prompt, options) => {
      const { includeOtherPrompts = false, maxCount = 4 } = options ?? {};

      const allDoodles = distinct_key(
        [
          ...summaryData.doodles,
          ...memory.doodles.map((x) => ({
            ...x,
            score: memory.doodleScores.find((s) => s.doodleKey === x.key)?.score ?? 0,
          })),
        ],
        (x) => x.key,
      );
