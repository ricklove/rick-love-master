...
export const DoodlePartyProfileView = (props: { controller: DoodlePartyController; onDone: () => void }) => {
  ...
  const [usedEmojis, setUsedEmojis] = useState(
    props.controller.meshState?.players.filter((x) => x.clientKey !== clientPlayer.clientKey).map((x) => x.emoji) ?? [],
  );
  ...
  return (
    <>
      <C.View_Panel>
        ...
        <UserProfileView userProfile={userProfile} onUserProfileChange={changeUserProfile} usedEmojis={usedEmojis} />
...
export const DoodlePartyPlayerList = (props: { controller: DoodlePartyController; hideInactive?: boolean }) => {
  ...
  return (
    <>
      <View>
        {props.controller.meshState?.players
          .filter((x) => !props.hideInactive || x.isActive)
          .map((x) => (
...
const UserProfileView = ({
  ...
  onUserProfileChange: (value: UserProfileData) => void;
  usedEmojis: string[];
}) => {
      ...
      <C.View_Form>
        <C.View_FieldRow>
          ...
          <UserProfileName
            ...
            onNameChange={(x) => onUserProfileChange({ ...userProfile, name: x })}
          />
        </C.View_FieldRow>
      </C.View_Form>
    </>
...
const UserProfileName = ({
  ...
  return (
    <>
      ...
      />
      {isEditing && <C.Button_FieldInline onPress={changeName}>Set Name</C.Button_FieldInline>}
    </>
  );
};
