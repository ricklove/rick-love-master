...
import { EmojiIdleEmotionKind, EmojiIdleService, EmojiIdleState } from './emoji-idle-service';
...
export const EmojiIdleView = (props: {}) => {
  ...
  if (isExpanded && gameState) {
    return <TownView gameState={gameState} onClose={() => setIsExpanded(false)} />;
  }
...
const TownView = ({ gameState, onClose }: { gameState: EmojiIdleState; onClose: () => void }) => {
  ...
  const townPersons = gameState.townState.characters
    .map((x) => ({
      character: x,
  ...
  const missingSkills = skillTree.allNodes.filter(
    (x) => x.children.length <= 0 && !gameState.townState.characters.find((t) => t.characterEmoji === x.emoji),
  );
  const townMoney = townPersons.reduce((out, x) => {
    out += x.money ?? 0;
    return out;
  }, 0);
  return (
      ...
      {missingSkills.map((x) => (
        <View style={{ background: `#555555`, flexDirection: `row`, alignItems: `center` }}>
          <View style={{ position: `relative`, width: 100, height: 40, opacity: 0.5 }}>
            <EmojiCharacterView emoji={x.emoji} targetEmoji='' emotion={null} purchased={[]} />
          </View>
          <View style={{ padding: 4 }}>
          ...
          {targetOptionsLength > listSize && (
            <TouchableOpacity
              onPress={() =>
                setOffset((s) => {
                  const i = s + 5;
                  return i >= targetOptionsLength ? 0 : i;
                })
              }
            >
              <View>
            ...
            <TouchableOpacity key={x.emoji} onPress={() => EmojiIdleService.get().selectOption(x.emoji)}>
              <View
                style={{
                  ...
                  alignItems: `center`,
                  background: canBuy ? `#003300` : `#553300`,
                  borderRadius: 4,
                  paddingLeft: 2,
                  paddingRight: 2,
                }}
              >
                ...
                <Text
                  style={{ ...styles.costText_small, ...(canBuy ? {} : { color: `#FF0000` }) }}
                >{`$${x.cost.toLocaleString()}`}</Text>
              </View>
            </TouchableOpacity>
          );
        })}
  ...
  return (
    <View style={{ position: `absolute`, top: 0, right: 4 }}>
      <View style={{ flexDirection: `row`, justifyContent: `flex-end` }}>
        <View
          style={{
            transform: `translate(-${moneySizeOffset}px,${2 * moneySizeOffset}px) scale(${moneySizeOffset * 0.1 + 1})`,
          }}
        >
        ...
        <View
          style={{
            transform: `translate(-${multiplierSizeOffset}px,${2 * multiplierSizeOffset}px) scale(${
              multiplierSizeOffset * 0.1 + 1
            })`,
          }}
        >
          <Text
          ...
          >
            {`x${(multiplier ?? 1).toLocaleString()}`.padStart(4, ` `)}
          </Text>
        </View>
      </View>
    </View>
...
const EmojiCharacterView = ({
  ...
  emoji: string;
  targetEmoji: string;
  emotion: null | EmojiIdleEmotionKind;
  purchased: string[];
  onPress?: () => void;
}) => {
  return (
  ...
  return (
    <>
      {purchased.map((x, i) => (
        <View
          key={x}
          style={{ position: `absolute`, left: 0, top: 0, flexDirection: `column`, alignItems: `flex-end` }}
        >
...
const EmotionView = ({ emotion, emoji }: { emotion: null | EmojiIdleEmotionKind; emoji: string }) => {
  ...
  return <Text style={display.isEmotion ? styles.emotionEmoji : styles.characterEmoji}>{display.text}</Text>;
};
