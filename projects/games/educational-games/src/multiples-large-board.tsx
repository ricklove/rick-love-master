      ...
      <View style={{ marginTop: 50, marginBottom: 150, padding: 2, alignItems: `center` }}>
        <View style={{ alignItems: `center` }}>
          ...
          <GameGamepadInput
            ...
            buttons={[{ text: `🔴`, onPress: submitAnswer }]}
          />
        </View>
      </View>
    </>
...
export const ProblemView = (props: { problem: number }) => {
  return (
    <View style={gameStyles.problemView}>
      <Text style={gameStyles.problemText}>{`${props.problem}`}</Text>
    </View>
  );
};
...
const GameBoard = ({ gameBoard }: { gameBoard: GameBoardState }) => {
  ...
  const getCellViewStyle = (cell: GameBoardCell) => {
    const { i, j, c, r } = cell;
    const focusState = focus.i === i && focus.j === j ? `row col` : focus.i === i ? `col` : focus.j === j ? `row` : ``;
    const unitSize = 4;
    ...
    const background = `
        repeating-linear-gradient(${lineColorRow} 0px, ${lineColorRow} ${lineRadius}px, ${solidColorRow} ${lineRadius}px, ${solidColorRow} ${
      unitSize - lineRadius
    }px, ${lineColorRow} ${unitSize - lineRadius}px, ${lineColorRow} ${unitSize}px), 
        repeating-linear-gradient(0.25turn, ${lineColorCol} 0px, ${lineColorCol} ${lineRadius}px, ${solidColorCol} ${lineRadius}px, ${solidColorCol} ${
      unitSize - lineRadius
    }px, ${lineColorCol} ${unitSize - lineRadius}px, ${lineColorCol} ${unitSize}px)
        `;
...
const GameGamepadInput = (props: {
  gameBoard: GameBoardState;
  onChangeFocus: (focus: GameBoardPosition) => void;
  ...
  return (
    <GamepadDiscrete
      style={{ backgroundColor: colors.buttonBackground, borderColor: colors.buttonBorder }}
      onMove={(dir) => move(dir.x, dir.y)}
