...
import { Problem, ProblemResult, ProblemService } from './problems-service';

export const createReviewProblemService = (
  ...
  const getReviewProblem = (): null | Problem => {
    ...
    if (!state.reviewSequence) {
      ...
      if (!reviewProblem) {
        console.log(`createReviewProblemService getReviewProblem - No Review Problem (with greater order than last)`, {
          reviewProblems: state.reviewProblems,
          lastReviewProblem: state.lastReviewProblem,
          state,
        });
