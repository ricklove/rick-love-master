...
import { ProblemAnswer, ProblemService } from '../problems-service';
import { getSpellingEntries } from './spelling-entries';

export const createSpellingProblemService = ({
  speechService,
  maxAnswers = 4,
  sectionSize = 25,
}: {
  ...
  const service: ProblemService = {
    ...
    getSections: () =>
      [...new Array(sectionCount)].map((x, i) => ({
        key: getSectionKey(i),
    ...
    getNextProblem: () => {
      ...
      const wrongValues = shuffle(distinct(p.mispellings)).slice(0, wrongAnswerCount);

      const answers: ProblemAnswer[] = shuffle([
        ...wrongValues.map((x) => ({ value: `${x}`, isCorrect: false })),
