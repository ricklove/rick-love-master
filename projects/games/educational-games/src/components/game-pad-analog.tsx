...
import { Platform, Pressable, Text, View } from '@ricklove/react-native-lite';

export type GamepadPressState = {
  moveDirection: { x: number; y: number };
  buttons: { key: string; text: string; isDown: boolean }[];
};
export const GamepadAnalogStateful = (props: {
  ...
  return (
    <GamepadAnalog
      {...props}
      onMovePressIn={onMovePressIn}
      onMovePressOut={onMovePressOut}
...
export const GamepadAnalog = (props: {
  ...
  const DirectionButton = ({ text, direction }: { text: string; direction: { x: number; y: number } }) => {
    return (
      <Pressable
        style={inputStyles.cellTouch}
        onPressIn={() => onMovePressIn(direction)}
        onPressOut={() => onMovePressOut(direction)}
      >
