  ...
  return (
    <EducationalGame_StarBlastSideways
      problemService={createAutoSavedProblemService(
        createReviewProblemService(createMultiplesProblemService({ min: 1, max: 12 }), {}),
        `ProblemsMultiples`,
      )}
    />
  );
...
export const EducationalGame_StarBlastSideways = (props: { problemService: ProblemService }) => {
  // Add Pet Feeder
  return (
    <EducationalGame_StarBlastSideways_Inner problemService={createProgressGameProblemService(props.problemService)} />
  );
};
          ...
          <View style={{ overflow: `hidden`, padding: 10, paddingTop: 4, paddingBottom: 64 }}>
            <View style={{ position: `relative` }}>
              <GameView
                ...
                problemSourceKey={`${problemSourceKey}`}
              />
              ...
              <View style={{ zIndex: 10, flex: 1, alignSelf: `stretch` }}>
                <GamepadAnalogStateful
                  style={colors.gamepad}
...
const GameView = (props: {
  pressState: GamepadPressState;
  pauseState: { paused: boolean };
  problemService: ProblemService;
  problemSourceKey: string;
}) => {
  ...
  const playerPositionState = useRef({
    y: gameStyles.viewscreenView.height * 0.5,
    x: gameStyles.viewscreenView.width * 0.15,
    rotation: 0,
  } as GamePosition);
  ...
  const problemsState = useRef(
    null as null | {
      problemTime: number;
      question: string;
      answers: (Problem['answers'][0] & { key: string; pos: GamePosition; isAnsweredWrong: boolean })[];
    },
  );
  ...
  const restartGame = () => {
    // Move enemies back
    enemiesState.current.enemies.forEach((e2) => {
      e2.pos.x += gameStyles.viewscreenView.width * 0.5;
    });
  ...
  const gotoNextProblem = () => {
    ...
    const newProblemState = {
      ...
      question: p.question,
      answers: p.answers.map((x, i) => ({
        ...x,
        key: `${p.question} ${x.value}`,
        pos: {
          y: pSize * (0.5 + i),
          x: gameStyles.viewscreenView.width - gameStyles.sprite.viewSize.width * 0.5,
          rotation: 0,
        },
    ...
    const newEnemyState = {
      enemies: newProblemState.answers.map((ans, i) => {
        const enemy = {
          ...
          pos: {
            y: pSize * (0.5 + i),
            x: gameStyles.viewscreenView.width - gameStyles.sprite.viewSize.width * 1.5,
            rotation: 0,
          },
          vel: { x: -5, y: 0 },
          onHit: () => {
            setTimeout(() => {
              ...
              if (ans.isCorrect) {
                ...
                // Let bullets clear
                projectilesState.current.shots.forEach((x) => {
                  x.ignore = true;
                });
                gotoNextProblem();
    ...
    const gameLoop = () => {
      ...
      const getCommonState = (): CommonGameState => {
        return {
          ...
          onLoseLife: () => {
            if (gameState.current.deadTime || gameState.current.gameOverTime) {
              return;
            }

            projectilesState.current.debris.push({
              key: `player${gameTime}`,
              kind: `player-character`,
              pos: { ...playerPositionState.current },
              vel: { x: 0, y: 0 },
            });
      ...
      } else if (getGameTime().gameTime > 1 + gameState.current.gameOverTime && pressState.current.buttons[0]?.isDown) {
        setTimeout(() => {
          restartGame();
        }, 250);
      }
  ...
  const directAnswer = (enemy: EnemiesState['enemies'][0]) => {
    ...
    if (gameState.current.deadTime) {
      return;
    }
    if (enemy.destroyed || enemy.explodeTime) {
      return;
    }
  ...
  return (
    <>
      <View style={gameStyles.viewscreenView}>
        ...
        {[...new Array(gameState.current.lives)].map((x, i) => (
          <Sprite
            ...
            kind='life'
            position={{
              x: gameStyles.viewscreenView.width - gameStyles.sprite.viewSize.width * (1 + i),
              y: gameStyles.viewscreenView.height - 0.5 * gameStyles.sprite.viewSize.height,
              rotation: 0,
            }}
          />
        ))}
        ...
        {enemiesState.current?.enemies
          .filter((e) => !e.destroyed)
          .map((e) => (
            <React.Fragment key={e.key}>
              {!e.answer.isAnsweredWrong && (
                <TextPositioned
                  position={{ y: e.pos.y, x: e.pos.x + gameStyles.sprite.viewSize.width, rotation: 0 }}
                  text={e.answer.value}
                />
              )}
              {e.answer.isAnsweredWrong && (
        ...
        {enemiesState.current?.enemies
          ...
          .map((x) => (
            <React.Fragment key={x.key}>
              ...
              {x === gameState.current.weaponsLock?.lockedEnemy && <Sprite kind='lock' position={x.pos} />}
            </React.Fragment>
          ))}
        ...
        {gameState.current.gameOverTime && (
          <View>
            <View
              style={{
                position: `absolute`,
                top: gameStyles.viewscreenView.height * 0.5,
                width: gameStyles.viewscreenView.width,
              }}
            >
              <Text style={gameStyles.gameOver.text}>Game Over</Text>
              {getGameTime().gameTime > 1 + gameState.current.gameOverTime && (
        ...
        <View style={gameStyles.directAnswerButtonContainer}>
          {enemiesState.current?.enemies.map((e) => (
            <TouchableOpacity key={e.key} onPress={() => directAnswer(e)}>
              <View
                style={
                  !e.destroyed && !e.explodeTime ? gameStyles.directAnswerButton : gameStyles.directAnswerButton_hidden
                }
              />
            </TouchableOpacity>
          ))}
      ...
      <View style={{ position: `relative`, height: gameStyles.question.text.fontSize * 3, alignSelf: `stretch` }}>
        <TextPositioned
          text={problemsState.current?.question ?? ``}
          position={{
            x: gameStyles.viewscreenView.width * 0.25,
            y:
              gameStyles.question.text.fontSize * 1 -
              Math.max(0, gameStyles.viewscreenView.height * 0.5 - 125 * timeSinceProblem),
            rotation: 0,
          }}
        />
      </View>
...
const updateGame = ({
  ...
  if (gameState.deadTime && gameTime > 3 + gameState.deadTime) {
    ...
    enemiesState.enemies.forEach((e2) => {
      e2.pos.x += gameStyles.viewscreenView.width * 0.5;
    });
...
const updatePlayer = ({
  ...
  playerPosition,
  gameState,
}: CommonGameState): { playerPosition: GamePosition } => {
  if (gameState.deadTime) {
  ...
  const dir =
    dirPress.x || dirPress.y
      ? { x: dirPress.x, y: -dirPress.y }
      : targetPos
      ? { x: targetPos.x - playerPosition.x, y: targetPos.y - playerPosition.y }
      : { x: 0, y: 0 };
...
type ProjectilesState = {
  ...
  shots: {
    ...
    explodeTime?: number;
    ignore?: boolean;
    lockedEnemy?: { pos: GamePosition; explodeTime?: number; destoyed?: boolean };
  }[];
...
const updateProjectiles = ({
  ...
  gameState,
  onLoseLife,
}: CommonGameState): ProjectilesState => {
...
const updateEnemies = ({
  ...
  enemies.forEach((e) =>
    shots.forEach((s) => {
      ...
      if (s.explodeTime || s.ignore) {
        return;
      }

      // Only shoot locked enemy
      if (s.lockedEnemy && s.lockedEnemy !== e) {
        return;
      }
  ...
  // Closest Enemy
  const closestEnemyToPlayer = enemiesState.enemies
    .filter((x) => !x.explodeTime && !x.destroyed)
...
const getSpriteEmoji = (kind: SpriteKind) => {
  ...
  switch (kind) {
    case `player`:
      return { text: `🚀`, rotation: +0.125, offsetX: 0, offsetY: -0.25 };
    case `player-character`:
      return { text: `😭` };
    case `player-character-splat`:
      return { text: `😫`, rotation: 0.15 };
    case `shot`:
      return { text: `🔥`, rotation: -0.25 };
    case `shot-explode`:
      return { text: `✨`, rotation: 0, scale: 0.5 };
    case `lock`:
      return { text: `💢`, offsetX: -0.25, offsetY: -0.125, scale: 1.5 };
    case `enemy`:
      return { text: `🛸`, offsetX: -0.125, offsetY: -0.125 };
    case `enemy-explode`:
      return { text: `💥`, offsetX: -0.125, offsetY: -0.125 };
    case `answer-wrong`:
      return { text: `❌`, offsetX: -0.125, offsetY: -0.125 };
    ...
    case `life`:
      return { text: `🚀`, scale: 0.5 };
    case `text`:
      return { text: ``, offsetX: 0, offsetY: -0.125 };
    default:
      return { text: `😀` };
  }
};
...
const Sprite = ({
  ...
  const styleRotation = {
    ...size,
    transform: `translate( ${size.width * -0.5 + Math.floor((s.offsetX ?? 0) * fontSize)}px, ${
      size.height * -0.5 + Math.floor((s.offsetY ?? 0) * fontSize)
    }px) rotate(${s.rotation ?? 0}turn) scale(${s.scale ?? 1})`,
  };
  return (
