...
import { randomIndex } from '@ricklove/utils-core';
...
export const EducationalGame_MultiplesSnake = (props: {}) => {
  ...
  const onMove = (value: { col: number; row: number }) => {
    ...
    const oldPlayerCell =
      newGameBoard.columns[newGameBoard.player.position.col].cells[newGameBoard.player.position.row];
...
const updateBoard = (boardRaw: GameBoardState) => {
  ...
  const getConnection = (
    m: { position: GameBoardPosition },
    lastBody?: { position: GameBoardPosition },
    nextBody?: { position: GameBoardPosition },
  ) => {
    const connected = {
      t:
        (m.position.col === lastBody?.position.col && m.position.row < lastBody?.position.row) ||
        (m.position.col === nextBody?.position.col && m.position.row < nextBody?.position.row),
      b:
        (m.position.col === lastBody?.position.col && m.position.row > lastBody?.position.row) ||
        (m.position.col === nextBody?.position.col && m.position.row > nextBody?.position.row),
      l:
        (m.position.row === lastBody?.position.row && m.position.col > lastBody?.position.col) ||
        (m.position.row === nextBody?.position.row && m.position.col > nextBody?.position.col),
      r:
        (m.position.row === lastBody?.position.row && m.position.col < lastBody?.position.col) ||
        (m.position.row === nextBody?.position.row && m.position.col < nextBody?.position.col),
    };
  ...
  board.body.forEach((m, i) => {
    updateBoardPosition(
      ...
      `body`,
      board.body.length - i,
      getConnection(m, board.body[i - 1], board.body[i + 1] ?? board.player),
    );
  });
  ...
  updateBoardPosition(
    ...
    `player`,
    0,
    getConnection(board.player, board.body[board.body.length - 1], undefined),
  );
};

const updateBoardPosition = (
  boardRaw: GameBoardState,
  position: GameBoardPosition,
  kind: 'player' | 'body',
  bodyIndex: number,
  connected: {
        ...
        <View style={{ flexDirection: `column-reverse` }}>
          ...
          {gameBoard.columns[0].cells.map((r) => (
            <View key={r.row} style={focus.row === r.row ? styles.focusCellHeaderView : styles.cellHeaderView}>
              <Text style={focus.row === r.row ? styles.focusCellHeaderText : styles.cellHeaderText}>{`${
                r.row + 1
              }`}</Text>
            </View>
          ))}
        ...
        {gameBoard.columns.map((c) => (
          <View key={c.col} style={{ flexDirection: `column-reverse` }}>
            <View style={focus.col === c.col ? styles.focusCellHeaderView : styles.cellHeaderView}>
              <Text style={focus.col === c.col ? styles.focusCellHeaderText : styles.cellHeaderText}>{`${
                c.col + 1
              }`}</Text>
            </View>
            {c.cells.map((cell) => (
              <View
                ...
                style={
                  cell.state !== `blank`
                    ? [styles.focusCellView, getCellStyle(boardTick, cell, gameBoard.body.length)]
                    : styles.cellView
                }
              >
...
const createGameInputState = (
  ...
  onMove: (value: { col: number; row: number }) => void,
): GameInputState => {
  ...
  const buttons = nextCells.map((x) => ({
    key: `${x.row} ${x.col}`,
    text: `${x.value}${
      isOnSquare && player.position.col === x.col ? `↕` : isOnSquare && player.position.row === x.row ? `↔` : ``
    }`,
    onPress: () => onMove(x),
  }));
