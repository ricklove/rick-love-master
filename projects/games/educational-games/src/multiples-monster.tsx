...
const createDefaultGameBoardState = (): GameBoardState => {
  const gameBoard: GameBoardState = {
    ...
    monsters: [...new Array(3)].map((x) => ({
      position: { col: randomIndex(maxMultiple), row: randomIndex(maxMultiple) },
    })),
  };
        ...
        {gameBoard.columns.map((c) => (
          <View key={c.col} style={{ flexDirection: `column-reverse` }}>
            ...
            {c.cells.map((cell) => (
              <View
                key={cell.row}
                style={focus.row >= cell.row && focus.col >= c.col ? styles.focusCellView : styles.cellView}
              >
