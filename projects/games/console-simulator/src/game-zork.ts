  ...
  execute: async () => {
    const mainZork: ConActionQuery = {
      ...
      respond: async ({ command, target }) => {
        if (command === `zork`) {
          return { output: `At your service`, query: mainZork };
        }
        if (command === `jump`) {
          return {
            output:
              Math.random() < 0.5 ? `Are you enjoying yourself?` : `Very good! Now you can go to the second grade.`,
            query: mainZork,
          };
        }
        if (command === `scream`) {
          return { output: `Aaaarrrrgggghhhh!`, query: mainZork };
        }
