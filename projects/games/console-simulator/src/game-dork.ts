import { createDorkGame, GameAction, GameInput, title } from '@ricklove/dork';
import { ConActionQuery, ConFile } from './types';
...
const loadGameStorage = () => {
  ...
  try {
    return JSON.parse(v) as DorkGameStorage | null;
  } catch {
    return null;
  }
};
const saveGameStorage = (value: DorkGameStorage) => {
  localStorage.setItem(`DorkGameStorage`, JSON.stringify(value));
};
