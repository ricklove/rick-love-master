...
export const CodeWrapper_ZoomButtons = (props: { children: ReactNode; adjustWidth?: boolean }) => {
  ...
  return (
    <div className='code-wrapper' style={{ position: `relative` }}>
      ...
      <div style={{ position: `absolute`, top: 4, right: 64, height: 16, width: 16 }} onClick={() => zoomScale(`out`)}>
        <span>{`${`➖`}`}</span>
      </div>
      <div style={{ position: `absolute`, top: 4, right: 32, height: 16, width: 16 }} onClick={() => zoomScale(`in`)}>
        <span>{`${`➕`}`}</span>
      </div>
    </div>
  );
