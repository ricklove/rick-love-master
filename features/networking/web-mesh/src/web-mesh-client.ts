import { AppError, createSubscribable, distinct_key } from '@ricklove/utils-core';
import { createWebsocketConnection_smart, WebsocketsApiConfig } from '@ricklove/websockets-api-client';
...
const createWebMeshClient_websocketOnly = <TMeshState, TMeshMessage>({
  ...
  reduceClientsState: (
    previousState: TMeshState,
    clients: { key: string; lastActivityTimestamp: Timestamp }[],
  ) => TMeshState;
  websocketsApiConfig: WebsocketsApiConfig;
}) => {
  const WEB_MESH_CLIENT_KEY = `_webMeshClientKey`;
  const clientKey = (localStorage.getItem(WEB_MESH_CLIENT_KEY) ??
    `${Date.now()}-${Math.floor(Math.random() * 999999)}`) as ClientKey;
  ...
  let sendSyncStateTimeoutId = setTimeout(() => {
    /* Empty */
  }, 0);
  const websocket = createWebsocketConnection_smart<WebSocketMessageWithSenderInfo>(
    { websocketsApiUrl: websocketsApiConfig.websocketsApiUrl, channelKey: `wm_${channelKey}` },
    (message) => {
      ...
      if (message.kind === `drop`) {
        clearTimeout(pingPongDropTimeoutId ?? 0);
        pingPongDropTimeoutId = null;
        state.meshMetaData.clients = state.meshMetaData.clients.filter(
          (x) => !message.clientKeys.some((c) => c === x.key),
        );
        state.meshState = reduceClientsState(state.meshState, state.meshMetaData.clients);
        stateSub.onStateChange(state.meshState);
        return;
      ...
      if (message.kind === `sync`) {
        ...
        if (
          state.meshMetaData.firstMessageTimestamp &&
        ...
        state.meshMetaData.clients = distinct_key(
          [...state.meshMetaData.clients, ...message.state.meshMetaData.clients],
          (x) => x.key,
        );
  ...
  const sendMessage = (message: TMeshMessage) => {
    if (isClosed) {
      throw new AppError(`The connection is closed`);
    }
