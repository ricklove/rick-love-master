import {
  ...
  UploadApiConfig,
  UploadUrl,
} from '@ricklove/upload-api-client';
...
const createUserDataService = (uploadApiConfig: UploadApiConfig) => {
  ...
  const storage = {
    ...
    clearUserData: () => {
      ...
      state.userProfiles.forEach((x) => {
        x.lastUploadTimestamp = undefined;
      });
  ...
  const uploadBackup = async (userState: UserProfileData, userData: UserData) => {
    const uploadUrl = await uploadApi.createUploadUrl({
      prefix: `backup/users/${userState.name}/${userState.uploadUrl.relativePath}/${Date.now()}`,
    });
...
const serviceState = {
  uploadApiConfig: null as null | UploadApiConfig,
  instance: null as null | UserDataService,
};
export const UserDataService = {
  setup: (uploadApiConfig: UploadApiConfig) => {
    serviceState.uploadApiConfig = uploadApiConfig;
  },
  get: (): UserDataService => {
    if (!serviceState.uploadApiConfig) {
      throw new AppError(`UserDataService is not setup: Call UserDataService.setup`);
    }
    if (!serviceState.instance) {
      serviceState.instance = createUserDataService(serviceState.uploadApiConfig);
    }
