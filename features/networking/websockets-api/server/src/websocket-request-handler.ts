...
import { distinct, groupItems, toKeyValueArray } from '@ricklove/utils-core';
import { WebsocketConnectionData } from '@ricklove/websockets-api-common';
...
export const handleWebsocketEvent = async (event: WebsocketEvent) => {
  ...
  const connectionInfo = await getOrAddConnectionId_DynamoDb({
  ...
  const batches = toKeyValueArray(
    groupItems(
      connectionInfo.connectionIds.map((x, i) => ({ connectionId: x, batchId: i % BATCH_COUNT })),
      (x) => `${x.batchId}`,
    ),
  );
  await Promise.all(
      ...
      for (const item of batch.value.reverse()) {
        ...
        try {
          ...
          if (sendFailureIds.length > 50) {
            console.log(
              `handleWebsocketEvent - EARLY CLEAN - Many sendFailures found - Not able to send to all clients`,
              { x: connectionId },
            );
            return;
            // eslint-disable-next-line no-await-in-loop
  ...
  if (sendFailureIds.length > 0 || Date.now() > 5 * 60 * 1000 + (connectionInfo.lastConnectionCleanedTimestamp ?? 0)) {
...
const getOrAddConnectionId_DynamoDb = async (args: {
  websocketKey: string;
  connectionIds: string[];
}): Promise<ConnectionInfo> => {
  return await getAndUpdateConnectionInfo({ ...args, action: `add` });
};
const removeConnectionId_DynamoDb = async (args: {
  websocketKey: string;
  connectionIds: string[];
}): Promise<ConnectionInfo> => {
...
export const getAndUpdateConnectionInfo = async ({
  ...
  const allIds =
    action === `add`
      ? distinct([...existingConnectionIds, ...(connectionIds ?? [])])
      : distinct(existingConnectionIds.filter((x) => !connectionIds.includes(x)));
  ...
  if (existingConnectionIds.length !== allIds.length) {
    ...
    const valueJsonObj: ConnectionInfo = {
      ...
      lastConnectionCleanedTimestamp:
        action === `remove` ? Date.now() : existingConnectionInfo?.lastConnectionCleanedTimestamp,
    };
