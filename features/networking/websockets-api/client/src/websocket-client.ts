import { AppError, createSubscribable } from '@ricklove/utils-core';
import { WebsocketApi, WebsocketConnectionData, WebsocketConnectionEvent } from '@ricklove/websockets-api-common';
    ...
    connect: <T>({ channelKey }: WebsocketConnectionData) => {
      ...
      const createSocket = () => {
        ...
        socket.addEventListener(`error`, (event) => {
          ...
          subscribableEvents.onStateChange({
            connectionStatus: `error`,
            error: { message: `Websocket Error`, error: event },
          });
          reconnect();
        });
